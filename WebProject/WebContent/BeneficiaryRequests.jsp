<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="java.util.List,java.util.ArrayList,com.ts.dao.CustomerDAO,com.ts.dto.Customer,com.ts.dao.AccountsDAO,com.ts.dto.Accounts,com.ts.dto.Employee,com.ts.dao.EmployeeDAO,com.ts.dao.BeneficiaryDAO,com.ts.dto.Beneficiary"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<title>Samridhi Beneficiary Requests Page</title>
...
BeneficiaryDAO bene = new BeneficiaryDAO();
List<Beneficiary> beneficiary = bene.getAllBeneficiary();
List<Beneficiary> pendingBen = new ArrayList<Beneficiary>();
for(Beneficiary ben : beneficiary) {
	if(ben.getStatus().equalsIgnoreCase("Pending")) {
		pendingBen.add(ben);
	}
}
session.setAttribute("pendingBen", pendingBen);
%>
...
<div class="col-sm-8"> 
      ...
      <tr>
        ...
        <th></th>  
        <c:forEach var="ben" items="${pendingBen}">
                             
                        
        <tr>
        <td>${ben.customer.customerId}</td>
        <td>${ben.beneficiaryAccNumber}</td>
        <td><form action="RequestApproval.html" method = "post">
            <div class="form-check">
                 <button type="button" onclick="location.href='BenRequestsApproval.jsp?custId=${ben.customer.customerId}&benNo=${ben.beneficiaryAccNumber}'"/>View Request</button>
            </div>
        </form>   </td>
        </tr>
        </c:forEach>
