<%@page import="com.ts.dto.Beneficiary"%>
...
<% int cId = (Integer)session.getAttribute("customerId");
   ...
   for ( Beneficiary  benef: beneficiary) {
	   if(benef.getStatus().equalsIgnoreCase("Approved")) {
		   validbeneficiary.add(benef);
	   }
   }
...
Welcome <%=customer.getFirstName() + " " + customer.getLastName()%></font>
  ...
  <a class="active" href="DeleteBeneficiary.jsp">Delete Beneficiaries</a>
</div><br/>
 <form action="DeleteBeneficiariesServlet" method = "post">
    
    <div class="row">
                    <label for="Account Type" class="col-sm-5 control-label">Select Beneficiary Account Number<font color="red">*</font></label>
                    <div class="col-sm-5">
                        <select class="dropdown text-secondary" name="benAccNo" placeholder="select Account Number" requiredstyle="width:275px;height:30px">
                ...
                <span style="display:inline-block; width: 500px;"></span> <button type="button" class="btn btn-sm btn-danger" action="DeleteBeneficiariesServlet">DELETE</button>
